...
import { useCrypto } from "../context/CoinContwxt";

const Home = () => {
    const {coins , loading , error , lastUpdate , currency} = useCrypto();
    ...
    useEffect(() => {
        if(coins.length > 0){
            setDisplayCoin(coins.slice(0,10));
        }
    }, [coins]);
  return (
    <div className="max-w-7xl mx-auto px-5 md:px-10 lg:px-20 py-16 flex flex-col gap-16 font-sans">
      ...
      <div className="flex flex-col items-center text-center gap-8 max-w-4xl mx-auto">
        ...
        <form onSubmit={searchHandler} className="w-full mt-4 flex flex-col sm:flex-row gap-4 justify-center items-stretch">
          <input
        ...
        <table className="w-full min-w-[700px] text-left border-collapse border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-white">
          ...
          <tbody className="text-black font-bold text-base md:text-lg">
            {displayCoin.map((item, index) => (
              <tr key={item.id} className="border-b-2 border-black hover:bg-gray-100 transition-colors cursor-pointer group">
                ...
                <td className="py-6 px-6">
                  <div className="flex items-center gap-3 w-max">
                    <img 
                      src={item.image} 
                      alt={item.name} 
                      className="w-10 h-10 rounded-full border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] bg-white object-contain"
                    />
                ...
                <td className={`py-6 px-6 font-black ${item.price_change_percentage_24h >= 0 ? "text-green-600" : "text-red-600"}`}>
